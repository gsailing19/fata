...
| 命令 | 类型 | 是否联网 | 说明 |
|------|------|----------|------|
| `node tools/invariant-tests.js` | 本地 | 否 | 算法不变量，54 项 |
...
| `node tools/e2e-test.js` | 线上 | 是 | 真实 PoW + `/api/submit` + 匹配 + 通知 + 撤回，14 项断言，会修改线上池 |
...
- `tools/e2e-test.js` 已是完整匹配链路测试：走 `/api/submit`，断言匹配、关闭、Match Issue、双邮件通知和撤回。
